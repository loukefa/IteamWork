<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="看板名称" prop="kanbanmingcheng">
          <el-input v-model="ruleForm.kanbanmingcheng" 
              placeholder="看板名称" clearable  :readonly="ro.kanbanmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="看板名称" prop="kanbanmingcheng">
              <el-input v-model="ruleForm.kanbanmingcheng" 
                placeholder="看板名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.yulantu" label="预览图" prop="yulantu">
          <file-upload
          tip="点击上传预览图"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.yulantu?ruleForm.yulantu:''"
          @change="yulantuUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.yulantu" label="预览图" prop="yulantu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.yulantu.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="网页链接" prop="wangyelianjie">
          <el-input v-model="ruleForm.wangyelianjie" 
              placeholder="网页链接" clearable  :readonly="ro.wangyelianjie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="网页链接" prop="wangyelianjie">
              <el-input v-model="ruleForm.wangyelianjie" 
                placeholder="网页链接" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="内容说明" prop="neirongshuoming">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.neirongshuoming" label="内容说明" prop="neirongshuoming">
                    <span>{{ruleForm.neirongshuoming}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  data() {
	...
	neirongshuoming : false,
      ...
      rules: {
          ...
          wangyelianjie: [
                { required: true, message: '网页链接不能为空', trigger: 'blur' },
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='kanbanmingcheng'){
            this.ruleForm.kanbanmingcheng = obj[o];
	    this.ro.kanbanmingcheng = true;
            continue;
          }
          if(o=='yulantu'){
            this.ruleForm.yulantu = obj[o];
	    this.ro.yulantu = true;
            continue;
          }
          if(o=='wangyelianjie'){
            this.ruleForm.wangyelianjie = obj[o];
	    this.ro.wangyelianjie = true;
            continue;
          }
	...
	if(this.ruleForm.yulantu!=null) {
		this.ruleForm.yulantu = this.ruleForm.yulantu.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `kanbanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.kanbanxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    yulantuUploadChange(fileUrls) {
	this.ruleForm.yulantu = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
